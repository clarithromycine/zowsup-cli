...
class ReadDecoder:
    ...
    def getProtocolTreeNode(self, data) -> Any:
    ...
    def getToken(self, index, data) -> Any:
    ...
    def getTokenDouble(self, n, n2) -> Any:
        pos = n2 + n * 256
    ...
    def readInt8(self, data) -> Any:
        return data.pop(0);

    def readInt16(self, data) -> Any:
        intTop = data.pop(0)
        intBot = data.pop(0)
    ...
    def readInt20(self, data) -> Any:
         int1 = data.pop(0)
         int2 = data.pop(0)
         int3 = data.pop(0)
         return ((int1 & 0xF) << 16) | (int2 << 8) | int3

    def readInt24(self, data) -> Any:
        int1 = data.pop(0)
        int2 = data.pop(0)
        int3 = data.pop(0)
    ...
    def readListSize(self,token, data) -> Any:
        size = 0
        if token == 0:
            size = 0
        else:
    ...
    def readAttributes(self, attribCount, data) -> Any:
        attribs = {}
    ...
    def readString(self,token, data) -> Any:
        if token == -1:
        ...
        if token == 247:
            ...
            if xx==1:
                ...
                if device_no==0:
                    ...
                    jid = f"{user}:{device_no}@lid"
            else:
                #其余情况，暂时按照普通id处置
                jid = f"{user}.{xx}:{device_no}@s.whatsapp.net"
            return jid
    ...
    def readArray(self, length, data) -> Any:
        out = data[:length]
        del data[:length]
        return out

    def nextTreeInternal(self, data) -> Any:
    ...
    def readList(self,token, data) -> Any:
