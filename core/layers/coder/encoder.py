...
class WriteEncoder:
    ...
    def __init__(self, tokenDictionary) -> None:
        self.tokenDictionary = tokenDictionary

    def protocolTreeNodeToBytes(self, node) -> Any:
        outBytes = [0] # flags
        self.writeInternal(node, outBytes)                                
        return outBytes

    def writeInternal(self, node, data) -> Any:
        x = 1 + \
    ...
    def writeAttributes(self, attributes, data) -> Any:
    ...
    def writeBytes(self, bytes_, data, packed = False) -> Any:
        bytes__ = []
        for b in bytes_:
            if type(b) is int:
                bytes__.append(b)
    ...
    def writeInt8(self, v, data) -> Any:
        data.append(v & 0xFF)


    def writeInt16(self, v, data) -> Any:
        data.append((v & 0xFF00) >> 8)
        data.append((v & 0xFF) >> 0)

    def writeInt20(self, v, data) -> Any:
        data.append((0xF0000 & v) >> 16)
        data.append((0xFF00 & v) >> 8)
        data.append((v & 0xFF) >> 0)

    def writeInt24(self, v, data) -> Any:
        data.append((v & 0xFF0000) >> 16)
        data.append((v & 0xFF00) >> 8)
        data.append((v & 0xFF) >> 0)

    def writeInt31(self, v, data) -> Any:
        data.append((0x7F000000 & v) >> 24)
        data.append((0xFF0000 & v) >> 16)
        data.append((0xFF00 & v) >> 8)
        data.append((v & 0xFF) >> 0)

    def writeInt32(self, v, data) -> Any:
        data.append((0xFF000000 & v) >> 24)
        data.append((0xFF0000 & v) >> 16)
        data.append((0xFF00 & v) >> 8)
        data.append((v & 0xFF) >> 0)

    def writeListStart(self, i, data) -> Any:
        if i == 0:
            data.append(0)
        elif i < 256:
            data.append(248)
    ...
    def writeToken(self, token, data) -> Any:
    ...
    def encodeString(self, string) -> Any:
        res = []
    ...
    def writeJid(self, user, server, data) -> Any:
        if user.find(":") != -1:
            if server=="lid":
    ...
    def tryPackAndWriteHeader(self, v, headerData, data) -> Any:
        size = len(headerData)
        if size >= 128:
            return None
        arr = [0] * int((size + 1) / 2)
