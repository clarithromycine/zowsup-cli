...
class ConfigManager:
    ...
    def load(self, path_or_profile_name, profile_only=False) -> Any:
        ...
        if config is not None:
            ...
            logger.debug(f"Detecting config for profile={profile_name}, dir={config_dir}")
            ...
            logger.error("Could not find a config for profile={}, paths checked: {}".format(profile_name, ":".join(exhausted)))

    def _type_to_str(self, type) -> Any:
        """
        :param type:
        :type type: int
        :return:
    ...
    def guess_type(self, config_path) -> Any:
    ...
    def config_to_str(self, config, serialize_type=TYPE_JSON) -> Any:
    ...
    def save(self, profile_name, config, serialize_type=TYPE_JSON, dest=None) -> Any:
